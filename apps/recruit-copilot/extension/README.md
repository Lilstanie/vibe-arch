...
直接打开 `http://localhost:5178/mock-boss.html` —— 一个模拟 Boss 候选人页，已内置存储 shim 并加载
`content.js`。点右下角面板的「开场白」，就能看到：读取候选人 → 调 `/api/outreach` 取匹配分 + 话术 →
把开场白**填入**页面聊天框（不发送）→ 每日额度 +1。可切换不同候选人（含「未入库者」）验证各分支。
